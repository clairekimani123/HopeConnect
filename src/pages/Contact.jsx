...
const DonatePage = () => {
  ...
  const { user } = useAuth()
  const [formData, setFormData] = useState({ amount: '', phone: '', description: '', name: '', email: user.email,type: donationType, group:recipientGroup });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await fetch('https://hope-connect-backend-1-9syn.onrender.com/donations', {
            method: 'POST',
            headers: { 
        ...
        setFormData({ type: '', description: '', date: '', image_url: '' });
